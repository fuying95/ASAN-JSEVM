...
// fCorleone's code
const HackerRecord = require('./hackerInstructions.js')
...
module.exports = function (opts, cb) {
  ...
  function iterateVm (done) {
    ...
    function runOp (cb) {
      ...
      try {
        // run the opcode
        // fCorleone's code
        var hackerrecord = new HackerRecord()
        hackerrecord.Hacker_record(opName)
        ...
        if (e.errorType && e.errorType === 'VmError') {
          ...
          // if (e.substring(0, 1) === '*') {
          //   var fs = require('fs')
          //   var VulFunSig = runState.callData.toString('hex').substring(0, 8)
          //   e += 'the function signature is: ' + VulFunSig + '\n'
          //   // fs.writeFileSync("./solasan_report_"+VulFunSig,e);
          //   fs.appendFileSync('./solasan_report_' + VulFunSig, e)
          //   cb(e)
          //   return
          // }
          throw e
  ...
  function parseVmResults (err) {
    ...
    // hacker_state_stack.push(runState)
    if (err) {
      runState.logs = []
