...
function HackerGaslessSend (hacker_call_hashs, hacker_calls) {
  ...
  HackerGaslessSend.prototype.TestOracle = function () {                              //judge if the trade call suit the detection rule
    var hasException = false                                                          //if it has no exception, analyze the first information in “hack_call” that been called in next trade
  ...
  HackerGaslessSend.prototype.String = function () {
    return 'HackerGaslessSend fun: ' + this.feature.toString()                        //return the error message
  }
...
function HackerTimestampOp (hacker_call_hashs, hacker_calls) {
  ...
  // var HackerTimestampOpproto = HackerTimestampOp.prototype                         //inherited information from TestOracle
  HackerTimestampOp.prototype.TestOracle = function () {
    var rootCall = this.hacker_calls[0]
    if ((rootCall.OperationStack.toString()).indexOf('TIMESTAMP') >= 0) {             //analyze if the stack of operation code include any information of “TIMESTAMP”
      return true
    } else {
      return false
    }
  ...
  HackerTimestampOp.prototype.String = function () {
    return 'HackerTimestampOp fun: ' + this.hacker_calls[0].input.slice(0, 8)         //return the function signature
  }
...
function HackerNumberOp (hacker_call_hashs, hacker_calls) {
  ...
  // var HackerTimestampOpproto = HackerTimestampOp.prototype
  HackerNumberOp.prototype.TestOracle = function () {                                 //inherited information from TestOracle
    var rootCall = this.hacker_calls[0]
    if ((rootCall.OperationStack.toString()).indexOf('NUMBER') >= 0) {                //analyze if the stack of operation code include any information of “NUMBER”
      return true
    } else {
      return false
    }
  ...
  HackerNumberOp.prototype.String = function () {
    return 'HackerNumberOp fun: ' + this.hacker_calls[0].input.slice(0, 8)            //return the function signature
  }
...
function HackerDelegateCallInfo (hacker_call_hashs, hacker_calls) {
  ...
  HackerDelegateCallInfo.prototype.TestOracle = function () {                //judge if the trade call suit the detection rule  
    ...
    for (var i = 0; i < nextcalls.length; i++) {                             //the first information in “hacker call” saved the information of subsequence that a trade called
      var call = nextcalls[i]                                                //analysis the first information that be called in every trade
...
function HackerExceptionDisorder (hacker_call_hashs, hacker_calls) {
  ...
  HackerExceptionDisorder.prototype.TestOracle = function () {                         // judge if the trade call suit the detection rule  
    ...
    var nextcalls = this.hacker_calls[0].nextcalls                                     // analyse the subsequence the trade called,which saved in the first information of “hacker_call”
    for (var i = 0; i < nextcalls.length; i++) {                                       // analyse every trade
      var call = nextcalls[i]
      if (this.TriggerExceptionCall(this.hacker_calls[0], call)) {                     // judge if the information that TriggerExceptionCall returned is “true” or “false”
        this.hacker_exception_calls.push(call)                                         // if the ExceptionCall had be called, call hacker_exception_calls
        this.feature.push(call.input.slice(0, 8))                                      // return the function signature (automatic generate by EVM) 
...
return 'HackerExceptionDisorder fun: ' + this.feature.toString()                       // return the error message
  }
}
