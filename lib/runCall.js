...
module.exports = function (opts, cb) {
  ...
  var callcode = opts.callcode || false
  var isStatic = opts.static || false
  var hackercontractcall = new HackerContractCall()
  // var hacker_state_stack = hackercontractcall.get_hacker_state_stack()
      ...
      stateManager.clearContractStorage(createdAddress, function (err) {
        ...
        function newContractEvent (callback) {
          self.emit('newContract', {
            address: createdAddress, code: code
          }, callback)
        }
  ...
  function initHackerContractcall (cb) {
    var input
    if (txData != undefined) {
      input = txData.toString('hex')
    } else {
      input = ''
    }
    ...
    if (callcode) {
      if (caller != null) {
        ...
        // console.log('get init callcode')
        var nextCall = call.OnCallCode(hackerrunCodeOpts.caller, hackerrunCodeOpts.callee, hackerrunCodeOpts.gasLimit, hackerrunCodeOpts.input, hackerrunCodeOpts.value, stateManager)
      ...
      if (caller != null) {
        ...
        // console.log('get init delegatecall')
        var nextCall = call.OnDelegateCall(hackerrunCodeOpts.caller, hackerrunCodeOpts.callee, hackerrunCodeOpts.gasLimit, hackerrunCodeOpts.input, stateManager)
        // nextCall.snapshotId = snapshot ???
        if (nextCall == null) {
          console.log('nextcall is nil')
          cb()
      ...
      if (caller != null) {
        ...
        console.log('caller: ' + caller.toString('hex'))
        console.log('callee: ' + toAddress.toString('hex'))
        var nextCall = call.OnCall(hackerrunCodeOpts.caller, hackerrunCodeOpts.callee, hackerrunCodeOpts.gasLimit, hackerrunCodeOpts.input, hackerrunCodeOpts.value, stateManager)
        // nextCall.snapshotId = snapshot ???
        if (nextCall == null) {
          console.log('nextcall is nil')
          cb()
  ...
  function closeHackerContractcall (cb) {
    if (caller != null) {
      // console.log('\nclose call...')
      var hacker_call_stack = hackercontractcall.get_hacker_call_stack()
      if (hacker_call_stack != null) {
        var call = hacker_call_stack.pop()
        if (vmResults.exception === 0) {
          call.throwException = true
          if (vmResults.exceptionError == ERROR.OUT_OF_GAS) {
            call.gasException = true
          }
        }
      ...
      call.OnCloseCall(gasUsed, stateManager)
      if (hacker_call_stack.length === 1) {
        hackercontractcall.hacker_close()
      }
    }
    cb()
  }
