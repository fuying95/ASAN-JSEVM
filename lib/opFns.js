...
function printCrash (type_of_crash, a, b) {
  // console.log('********************************************************************************');
  // console.log('SOL_ASAN: '.red,type_of_crash);
  // console.log('the opreators are : \n'+a.toString()+'\n'+'and \n'+b.toString()+'\n');
  // console.log('*******************************************');
  var content = '********************************************************************************\n' +
    'SOL_ASAN: ' + type_of_crash + '\n' +
    'the opreators are : \n' + a.toString() + '\n' + 'and \n' + b.toString() + '\n'
  return content
  // var fs = require('fs');
  // fs.writeFileSync("./solasan_report_"+a.toString().substring(0,Math.min(10,a.toString().length)) + b.toString().substring(0,Math.min(10,b.toString().length)),content);
}

function detectOverflow (res) {
  var max = '115792089237316195423570985008687907853269984665640564039457584007913129639936'
...
module.exports = {
  ...
  ADD: function (a, b, runState) {
    // detect overflow
    var res = a.add(b)
    if (detectOverflow(res.toString())) {
      var content = printCrash('integer overflow when doing add opreation', a, b)
      runState.stopped = true
      throw content
    }
  ...
  MUL: function (a, b, runState) {
    ...
    var res = a.mul(b)
    if (detectOverflow(res.toString())) {
      var content = printCrash('integer overflow when doing mul opreation', a, b)
      runState.stopped = true
      throw content
    }
  ...
  SUB: function (a, b, runState) {
    // detect overflow
    var res = a.sub(b)
    if (detectOverflow(res.toString()) || res.negative === 1) {
      var content = printCrash('integer overflow when doing sub opreation', a, b)
      runState.stopped = true
      throw content
    }
  ...
  ADDMOD: function (a, b, c, runState) {
    ...
    var res = a.add(b)
    if (detectOverflow(res.toString())) {
      var content = printCrash('integer overflow when doing addmod opreation', a, b)
      runState.stopped = true
      throw content
    }
    if (c.isZero()) {
  ...
  MULMOD: function (a, b, c, runState) {
    ...
    var res = a.mul(b)
    if (detectOverflow(res.toString())) {
      var content = printCrash('integer overflow when doing mulmod opreation', a, b)
      runState.stopped = true
      throw content
    }
    if (c.isZero()) {
